...
sl_k2 <- silhouette(k2$cluster, dist(sdr_data_imputed))

final_sl_k2 <- fviz_cluster(sl_k2, data = sdr_data_imputed) + theme_minimal()
final_sl_k2
```
...
rf_matmort2 <- randomForest(normalized_score_sdg3_births ~ .,
                             data = sdr_data_imputed,
                             importance = TRUE)

importance_df_2 <- as.data.frame(rf_matmort2$importance)

importance_df_top_10_2 <- importance_df_2 %>%
  rownames_to_column(var = "variable") %>%
  slice_max(n = 10, order_by = `%IncMSE`)

#-- Plotting SDG 2
ggplot(importance_df_top_10_2, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Births",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")

#-- Prediction graph 1                    **>change this one<**
pdp::partial(rf_matmort2, pred.var = "normalized_score_sdg3_uhc", plot = TRUE)
#-- Prediction graph 2
pdp::partial(rf_matmort2, pred.var = "normalized_score_sdg6_sanita", plot = TRUE)
