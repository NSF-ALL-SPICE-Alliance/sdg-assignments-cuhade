...
goal_3_histogram <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  ...
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(title = "Correlation between education and helath",
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.75, smoothFactor = 0.25, color = ~colorQuantile("Greens", goal_1_score)(goal_1_score), label = mytext)
```